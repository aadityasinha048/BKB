...
/**
 * Send an email notification when a login account is locked out.
 * @param {string} toEmail - Recipient's email address
 * @param {string} identifier - Username/identifier that was locked out
 * @param {string} resetLink - Link to reset password / unlock account
 */
export async function sendLockoutEmail(toEmail, identifier, resetLink) {
  try {
    const transport = await getTransporter();
    ...
    const info = await transport.sendMail({
      from: FROM_ADDRESS,
      to: toEmail,
      subject: `Security Alert: Your Bihar Ka Bazaar Account has been Locked`,
      html: `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #FFFCF8; border: 1px solid #E8DDD4; border-radius: 16px; overflow: hidden;">
          <!-- Header -->
          <div style="background: #D32F2F; padding: 32px 28px; text-align: center;">
            <h1 style="color: #fff; font-size: 22px; margin: 0 0 4px;">⚠️ Security Notification</h1>
            <p style="color: rgba(255,255,255,0.8); font-size: 13px; margin: 0;">Bihar Ka Bazaar Security Alert</p>
          </div>

          <!-- Body -->
          <div style="padding: 28px;">
            <h2 style="color: #1A1410; font-size: 20px; margin: 0 0 12px;">Account Locked Out</h2>
            <p style="color: #4A3F35; font-size: 14px; line-height: 1.7; margin: 0 0 20px;">
              Hello,<br/><br/>
              Your account associated with <strong>${identifier}</strong> has been locked out for <strong>15 minutes</strong> due to 5 consecutive failed login attempts.
            </p>

            <div style="background: #FDFCEA; border: 1px solid #F9A825; border-radius: 12px; padding: 16px; margin-bottom: 24px;">
              <div style="font-size: 12px; font-weight: 700; color: #F9A825; text-transform: uppercase; margin-bottom: 6px;">Security Warning</div>
              <div style="font-size: 13px; color: #4A3F35;">
                If this wasn't you, someone may be attempting to access your account. We recommend securing your email and updating your credentials.
              </div>
            </div>

            <!-- Action Button -->
            <div style="text-align: center; margin-bottom: 24px;">
              <a href="${resetLink}" style="display: inline-block; background: #1B6B3A; color: #fff; padding: 12px 28px; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 14px; font-family: inherit;">
                Reset Password / Unlock Account
              </a>
            </div>

            <p style="color: #8C7B6E; font-size: 12px; line-height: 1.6; margin-top: 20px;">
              This link is valid for 1 hour. If you do not request a password reset, your account will automatically unlock in 15 minutes.
            </p>
          </div>
          ...
          <div style="background: #F5EEE6; padding: 16px 28px; text-align: center; border-top: 1px solid #E8DDD4;">
            <p style="color: #8C7B6E; font-size: 11px; margin: 0;">
              © ${new Date().getFullYear()} Bihar Ka Bazaar · Direct Farmer Trade
            </p>
          </div>
        </div>
      `,
    ...
    const previewUrl = nodemailer.getTestMessageUrl(info);
    if (previewUrl) {
      console.log('📧 Lockout email preview:', previewUrl);
    }
